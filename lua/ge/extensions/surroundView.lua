...
-- v1.1.2 REAR CAMERA ONLY + DYNAMIC GUIDELINES
-- Keeps corrected perspective (near = wide, far = narrow) while restoring
-- the original steering direction that matched BeamNG's steering input.
...
local cfg = {
  x = fp(-2.15), y = fp(0.00), z = fp(0.88),
  yaw = fp(180.0), pitch = fp(-16.0), roll = fp(0.0), fov = fp(118.0),
...
local function emitStatus(state,msg)
  if guihooks and guihooks.trigger then
    guihooks.trigger('SurroundViewStatus',{
      state=state,message=msg or state or 'UNKNOWN',
      mode='rear-camera-dynamic-guides',version='1.1.2'
    })
  end
end
...
local function displayedSteering()
  -- Keep BeamNG's original steering sign. This matched the guide direction correctly.
...
local function guidePoint(p0,w,h,t,side)
  ...
  -- Correct perspective: near side is wider, far side is narrower.
...
local function drawCrossBar(dl,p0,w,h,t,color,thickness)
  dl:AddLine(guidePoint(p0,w,h,t,-1),guidePoint(p0,w,h,t,1),color,thickness)
end
...
local function drawCalibration()
  im.Text('REAR CAMERA + DYNAMIC GUIDE CALIBRATION v1.1.2')
  ...
  im.Separator()
  im.Text(string.format('Live steering: %.3f | smoothed/displayed: %.3f',steeringRaw,displayedSteering()))
...
local function drawWindow()
  ...
  if open then
    im.Text(string.format('REAR CAMERA v1.1.2 - corrected perspective / original steering direction | steer %.2f',displayedSteering()))
    im.Separator()
    if im.BeginTabBar('rearTabs') then
      if im.BeginTabItem('CAMERA') then
        ...
        if id then
          ...
          im.Dummy(im.ImVec2(drawW,drawH)); im.Text('Rear camera texture unavailable')
        end
        im.EndTabItem()
      end
      if im.BeginTabItem('CALIBRATION') then drawCalibration(); im.EndTabItem() end
      im.EndTabBar()
    end
  end
  im.End()
...
function M.startRearCamera()
  ...
  emitStatus('live','REAR CAMERA + DYNAMIC GUIDES LIVE v1.1.2')
  return true
end
