...
-- v0.7.4 HQ: higher-resolution GPU RenderViews + dense calibrated bird's-eye cell warp.
local im = extensions.ui_imgui or ui_imgui
local imUtils = require('ui/imguiUtils')

-- 4x 1080p-ish render targets. This is intentionally much higher than the old 512x288.
local WIDTH, HEIGHT = 1280, 720
local FOV = math.rad(112)
local NEAR_CLIP, FAR_CLIP = 0.05, 500
-- Dense grid reduces the blocky staircase look of the AddImage compatibility warp.
local GRID = 64
...
local calibration = {
  ...
  left ={k1=-0.14,k2=0.030,flipU=false,flipV=false,rotate=-90,crop={u0=0.02,v0=0.23,u1=0.98,v1=0.96},quad={{0.00,0.10},{0.40,0.36},{0.40,0.64},{0.00,0.90}}},
  right={k1=-0.14,k2=0.030,flipU=true, flipV=false,rotate=90,crop={u0=0.02,v0=0.23,u1=0.98,v1=0.96},quad={{0.60,0.36},{1.00,0.10},{1.00,0.90},{0.60,0.64}}}
}
...
local function emitStatus(state,msg)
  if guihooks and guihooks.trigger then guihooks.trigger('SurroundViewStatus',{state=state,message=msg or state or 'UNKNOWN',mode='hq-calibrated-four-camera-cellwarp',version='0.7.4'}) end
end
...
local function basis(v)
  ...
  d,u=norm(d),norm(u); local r; local ok=pcall(function() r=d:cross(u) end); if not ok or not r then r=vec3(-d.y,d.x,0) end
  return p,d,u,norm(r)
end
local function setCam(cam,p,d,u)
  local q=quatFromDir(norm(d),u); cam.quat.x,cam.quat.y,cam.quat.z,cam.quat.w=q.x,q.y,q.z,q.w
...
local function updateCams()
  ...
  local ok,err=pcall(function()
    setCam(cameras.front,p+d*2.25+u*1.05, d-u*0.55,u); setCam(cameras.rear,p-d*2.25+u*1.05,-d-u*0.55,u)
    setCam(cameras.left,p-r*1.20+u*1.05,-r-u*0.58,u); setCam(cameras.right,p+r*1.20+u*1.05,r-u*0.58,u)
  end)
  if not ok then return false,'camera transform failed: '..tostring(err) end; return true
end
local function createViews()
  ...
  local ok,err=pcall(function()
    for _,cam in pairs(cameras) do
      cam.renderView=RenderViewManagerInstance:getOrCreateView(cam.view); cam.renderView.renderCubemap=false; cam.renderView.cameraMatrix=cam.matrix
      cam.renderView.resolution=resolution; cam.renderView.viewPort=viewport; cam.renderView.namedTexTargetColor=cam.tex; cam.renderView.frustum=frustum
    end
  end)
  if not ok then return false,'RenderView setup failed: '..tostring(err) end; return true
end
...
local function lensUV(c,u,v)
  u,v=rotateUV(c,u,v); if c.flipU then u=1-u end; if c.flipV then v=1-v end
...
local function proj(c,u,v,o,w,h)
  local q=c.quad; local xt=q[1][1]*(1-u)+q[2][1]*u; local yt=q[1][2]*(1-u)+q[2][2]*u
  local xb=q[4][1]*(1-u)+q[3][1]*u; local yb=q[4][2]*(1-u)+q[3][2]*u
  return o.x+(xt*(1-v)+xb*v)*w,o.y+(yt*(1-v)+yb*v)*h
end

-- HQ compatibility warp: many tiny cells + high-res source textures.
local function warpCells(dl,cam,c,o,w,h)
  ...
  for gy=0,GRID-1 do
    ...
    for gx=0,GRID-1 do
      ...
      -- Small overlap only; the old 2px overlap made each low-res cell visibly smear.
...
local function surround()
  local a=im.GetContentRegionAvail(); local s=math.max(320,math.min(a.x,a.y,900)); local w,h=s*0.78,s
  ...
  warpCells(dl,cameras.front,calibration.front,o,w,h); warpCells(dl,cameras.rear,calibration.rear,o,w,h); warpCells(dl,cameras.left,calibration.left,o,w,h); warpCells(dl,cameras.right,calibration.right,o,w,h)
  local cw,ch=w*0.22,h*0.40; local cx,cy=o.x+w*0.5,o.y+h*0.5; local c0,c1=im.ImVec2(cx-cw*0.5,cy-ch*0.5),im.ImVec2(cx+cw*0.5,cy+ch*0.5)
  dl:AddRectFilled(c0,c1,im.GetColorU322(im.ImVec4(0.12,0.15,0.18,1)),cw*0.30,0); dl:AddRect(c0,c1,im.GetColorU322(im.ImVec4(0.82,0.89,0.96,1)),cw*0.30,0,2)
  ...
  dl:AddRect(o,im.ImVec2(o.x+w,o.y+h),im.GetColorU322(im.ImVec4(0.12,0.55,1,0.95)),22,0,2); im.SetCursorScreenPos(o); im.Dummy(im.ImVec2(w,h))
end
local function calibrationPanel()
  im.Text('v0.7.4 HQ: 1280x720 x4 / 64x64 warp grid'); im.Separator()
  im.Text('Performance note: this mode renders four HD camera views continuously.')
  for n,c in pairs(calibration) do im.Text(string.format('%s  k1 %.3f  k2 %.3f  rot %d',string.upper(n),c.k1,c.k2,c.rotate or 0)) end
end
local function drawWindow()
  if not im or not running or (showWindow and not showWindow[0]) then return end
  im.SetNextWindowSize(im.ImVec2(1100,920),im.Cond_FirstUseEver)
  ...
  if open then
    im.Text('4x HD GPU cameras / dense calibrated bird\'s-eye v0.7.4 HQ'); im.Separator()
    if im.BeginTabBar('svTabs') then
      if im.BeginTabItem('360 VIEW') then im.Text('HQ live bird\'s-eye composite'); local ok,err=pcall(surround); if not ok then im.TextColored(im.ImVec4(1,0.35,0.35,1),'360 draw error: '..tostring(err)); emitStatus('error','360 draw error: '..tostring(err)) end; im.EndTabItem() end
      if im.BeginTabItem('4 CAMERAS') then local aw=im.GetContentRegionAvail().x; local tw=math.max(260,(aw-16)*0.5); local th=tw*HEIGHT/WIDTH; tile(cameras.front,tw,th,false); im.SameLine(); tile(cameras.rear,tw,th,true); tile(cameras.left,tw,th,false); im.SameLine(); tile(cameras.right,tw,th,true); im.EndTabItem() end
...
function M.startSurroundView()
  running=false; local ok,err=createViews(); if not ok then emitStatus('error','360 init failed: '..tostring(err)); return false end
  local cok,cerr=updateCams(); if not cok then emitStatus('error','360 camera failed: '..tostring(cerr)); return false end
  running=true; if showWindow then showWindow[0]=true end; emitStatus('live','HQ 4 CAMERA SURROUND LIVE v0.7.4'); return true
end
...
function M.onExtensionUnloaded()
  running=false
  if RenderViewManagerInstance and RenderViewManagerInstance.destroyView then for _,cam in pairs(cameras) do if cam.renderView then pcall(function() RenderViewManagerInstance:destroyView(cam.renderView) end) end; cam.renderView=nil end end
end
return M