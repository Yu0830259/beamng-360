...
-- v0.9.1 REAL AVM PROJECTION+
-- Ground-plane inverse projection with per-camera intrinsics/extrinsics and soft seam blending.
...
-- Vehicle-local: +X forward, +Y right, +Z up.
local defaults = {
...
local mapControls={front=fp(5.8),rear=fp(4.8),side=fp(3.6),carLength=fp(4.5),carWidth=fp(1.9),blend=fp(0.22)}
local running=false
...
local function emitStatus(state,msg)
  if guihooks and guihooks.trigger then guihooks.trigger('SurroundViewStatus',{state=state,message=msg or state or 'UNKNOWN',mode='calibrated-ground-projection-blend',version='0.9.1'}) end
end
...
local function basis(v)
  ...
  d,u=norm(d),norm(u); local r; local ok=pcall(function() r=d:cross(u) end)
...
local function localCameraBasis(c)
  ...
  rr,up=rotateAroundForward(rr,up,math.rad(c.roll[0]))
  return f,rr,up
end
...
local function setCam(cam,pos,dir,up)
  local q=quatFromDir(norm(dir),norm(up)); cam.quat.x,cam.quat.y,cam.quat.z,cam.quat.w=q.x,q.y,q.z,q.w
...
local function updateCams()
  ...
  local ok,err=pcall(function()
    for name,cam in pairs(cameras) do
      local c=controls[name]; local lf,_,lu=localCameraBasis(c)
      local pos=p+d*c.x[0]+r*c.y[0]+u*c.z[0]
      setCam(cam,pos,toWorldVec(d,r,u,lf),toWorldVec(d,r,u,lu))
      cam.renderView.frustum=Frustum.construct(false,math.rad(c.fov[0]),ASPECT,NEAR_CLIP,FAR_CLIP)
    end
  end)
...
local function createViews()
  ...
  local ok,err=pcall(function()
    for name,cam in pairs(cameras) do
      cam.renderView=RenderViewManagerInstance:getOrCreateView(cam.view); cam.renderView.renderCubemap=false
      cam.renderView.cameraMatrix=cam.matrix; cam.renderView.resolution=resolution; cam.renderView.viewPort=viewport
...
local function image(cam,size) local id=tex(cam); if not id then im.Dummy(size); return false end; im.Image(id,size,uv0,uv1); return true end
local function tile(cam,w,h) im.BeginGroup(); im.Text(cam.label); image(cam,im.ImVec2(w,h)); im.EndGroup() end
...
local function projectGround(name,X,Y)
  local c=controls[name]; local f,rr,up=localCameraBasis(c)
  local rx,ry,rz=X-c.x[0],Y-c.y[0],-c.z[0]
  local zc=rx*f.x+ry*f.y+rz*f.z; if zc<=0.06 then return nil end
  local xc=rx*rr.x+ry*rr.y+rz*rr.z; local yc=rx*up.x+ry*up.y+rz*up.z
  local tanV=math.tan(math.rad(c.fov[0])*0.5); local tanH=tanV*ASPECT
  local U=c.cx[0]+xc/(2*zc*tanH); local V=c.cy[0]-yc/(2*zc*tanV)
  U,V=distortUV(c,U,V)
  if U<0.005 or U>0.995 or V<0.005 or V>0.995 then return nil end
  local dist=math.sqrt(rx*rx+ry*ry+rz*rz)
  local viewScore=zc/math.max(dist,0.001)
  -- Penalize extreme image edges so neighboring cameras blend before ugly edge stretching.
  local edge=math.min(U,1-U,V,1-V)*2
  local score=viewScore*(0.40+0.60*math.max(0,math.min(1,edge)))
  return U,V,score
end

local cameraOrder={'front','rear','left','right'}
local function bestTwo(X,Y)
  local a,b=nil,nil
  for _,name in ipairs(cameraOrder) do
    local U,V,S=projectGround(name,X,Y)
    if U then
      local e={name=name,u=U,v=V,s=S}
      if not a or S>a.s then b=a; a=e elseif not b or S>b.s then b=e end
    end
  end
  return a,b
end
...
local function drawSample(dl,name,px,py,cw,ch,u0,v0,u1,v1,alpha)
  local id=tex(cameras[name]); if not id then return end
  local tint=im.GetColorU322(im.ImVec4(1,1,1,alpha))
  dl:AddImage(id,im.ImVec2(px-0.12,py-0.12),im.ImVec2(px+cw+0.12,py+ch+0.12),im.ImVec2(u0,v0),im.ImVec2(u1,v1),tint)
end

local function drawProjectedGround(dl,o,w,h)
  local cellW,cellH=w/GRID_X,h/GRID_Y
  for gy=0,GRID_Y-1 do
    for gx=0,GRID_X-1 do
      local Xc,Yc=groundAt(gx+0.5,gy+0.5)
      local a,b=bestTwo(Xc,Yc)
      if a then
        ...
        if au0 and au1 then
          local px,py=o.x+gx*cellW,o.y+gy*cellH
          local blend=0
          if b and mapControls.blend[0]>0 then
            local ratio=b.s/math.max(a.s,1e-5)
            blend=math.max(0,math.min(0.48,(ratio-(1-mapControls.blend[0]))/math.max(mapControls.blend[0],0.001)*0.48))
          end
          drawSample(dl,a.name,px,py,cellW,cellH,au0,av0,au1,av1,1)
          if b and blend>0.01 then
            local bu0,bv0=projectGround(b.name,X0,Y0); local bu1,bv1=projectGround(b.name,X1,Y1)
            if bu0 and bu1 then drawSample(dl,b.name,px,py,cellW,cellH,bu0,bv0,bu1,bv1,blend) end
          end
        end
      end
    end
...
local function drawVehicleMask(dl,o,w,h)
  local totalX=mapControls.front[0]+mapControls.rear[0]; local totalY=mapControls.side[0]*2
  local carW=w*(mapControls.carWidth[0]/totalY); local carH=h*(mapControls.carLength[0]/totalX)
  local centerY=o.y+h*(mapControls.front[0]/totalX); local cx=o.x+w*0.5
  local c0=im.ImVec2(cx-carW*0.5,centerY-carH*0.5); local c1=im.ImVec2(cx+carW*0.5,centerY+carH*0.5)
...
local function surround()
  local a=im.GetContentRegionAvail(); local h=math.max(380,math.min(a.y,760))
  local physicalAspect=(mapControls.side[0]*2)/(mapControls.front[0]+mapControls.rear[0]); local w=math.min(a.x,h*physicalAspect); h=w/physicalAspect
  ...
  drawProjectedGround(dl,o,w,h); drawVehicleMask(dl,o,w,h)
...
local function resetCamera(name)
  ...
  c.x[0],c.y[0],c.z[0],c.yaw[0],c.pitch[0],c.roll[0],c.fov[0],c.cx[0],c.cy[0],c.k1[0],c.k2[0]=d.x,d.y,d.z,d.yaw,d.pitch,d.roll,d.fov,d.cx,d.cy,d.k1,d.k2
end
local function slider(label,ptr,a,b,fmt) im.SetNextItemWidth(280); im.SliderFloat(label,ptr,a,b,fmt or '%.2f') end

local function cameraEditor(name)
  local c=controls[name]; im.Separator(); im.Text(string.upper(name)..' CAMERA')
  slider('X Forward##'..name,c.x,-4,4,'%.2f m'); slider('Y Right##'..name,c.y,-2.5,2.5,'%.2f m'); slider('Z Height##'..name,c.z,0.2,2.5,'%.2f m')
  slider('Yaw##'..name,c.yaw,-180,180,'%.1f deg'); slider('Pitch##'..name,c.pitch,-85,10,'%.1f deg'); slider('Roll##'..name,c.roll,-45,45,'%.1f deg')
  slider('FOV##'..name,c.fov,70,130,'%.1f deg'); slider('Center X##'..name,c.cx,0.40,0.60,'%.3f'); slider('Center Y##'..name,c.cy,0.40,0.60,'%.3f')
  slider('Lens k1##'..name,c.k1,-0.35,0.35,'%.4f'); slider('Lens k2##'..name,c.k2,-0.20,0.20,'%.4f')
  if im.Button('Reset '..string.upper(name)) then resetCamera(name) end
end

local function calibrationTab()
  im.Text('AVM CALIBRATION v0.9.1 - EXTRINSICS + INTRINSICS + SEAM BLEND')
  im.Text('Use straight grid lines: adjust position/yaw/pitch/roll first, then FOV/center. Leave k1/k2 near 0 unless needed.')
  cameraEditor('front'); cameraEditor('rear'); cameraEditor('left'); cameraEditor('right')
  im.Separator(); im.Text('BIRD-EYE / STITCH')
  slider('Forward coverage',mapControls.front,2.5,10,'%.2f m'); slider('Rear coverage',mapControls.rear,2.5,10,'%.2f m'); slider('Side coverage',mapControls.side,2.0,7,'%.2f m')
  slider('Vehicle length mask',mapControls.carLength,3.0,6.5,'%.2f m'); slider('Vehicle width mask',mapControls.carWidth,1.4,2.8,'%.2f m')
  slider('Seam blend width',mapControls.blend,0.0,0.60,'%.2f')
...
local function drawWindow()
  ...
  im.SetNextWindowSize(im.ImVec2(1060,920),im.Cond_FirstUseEver)
  ...
  if open then
    im.Text('4-camera AVM / calibrated inverse projection + seam blend v0.9.1'); im.Separator()
    if im.BeginTabBar('svTabs') then
      if im.BeginTabItem('360 VIEW') then
        im.Text('Ground-plane projection with edge-aware two-camera seam blending.')
      ...
      if im.BeginTabItem('4 CAMERAS') then
        ...
        tile(cameras.front,tw,th); im.SameLine(); tile(cameras.rear,tw,th); tile(cameras.left,tw,th); im.SameLine(); tile(cameras.right,tw,th)
        im.EndTabItem()
      end
...
function M.startSurroundView()
  running=false; local ok,err=createViews(); if not ok then emitStatus('error',err); return false end
  local cok,cerr=updateCams(); if not cok then emitStatus('error',cerr); return false end
  running=true; if showWindow then showWindow[0]=true end; emitStatus('live','CALIBRATED GROUND PROJECTION + BLEND LIVE v0.9.1'); return true
end
