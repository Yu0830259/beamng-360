...
local function warpCells(dl,cam,c,o,w,h)
  ...
  for gy=0,GRID-1 do
    ...
    for gx=0,GRID-1 do
      ...
      local minx=math.min(x1,x2,x3,x4)-0.45
      local maxx=math.max(x1,x2,x3,x4)+0.45
      local miny=math.min(y1,y2,y3,y4)-0.45
      local maxy=math.max(y1,y2,y3,y4)+0.45
  ...
  if open then
    ...
    if im.BeginTabBar('svTabs') then
      ...
      if im.BeginTabItem('4 CAMERAS') then
        ...
        local tw=math.max(240,(aw-16)*0.5)
