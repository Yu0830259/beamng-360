...
-- v0.7.3: four persistent GPU RenderViews + improved calibrated bird's-eye cell warp.
...
-- rotate: 0, 90, -90 degrees in source UV space.
-- Quads are widened so the displayed ground is less zoomed-in around the vehicle.
local calibration = {
  front={k1=-0.12,k2=0.025,flipU=false,flipV=false,rotate=0,
    crop={u0=0.02,v0=0.28,u1=0.98,v1=0.94},
    quad={{0.18,0.00},{0.82,0.00},{0.66,0.43},{0.34,0.43}}},
  rear ={k1=-0.12,k2=0.025,flipU=true, flipV=false,rotate=0,
    crop={u0=0.02,v0=0.28,u1=0.98,v1=0.94},
    quad={{0.34,0.57},{0.66,0.57},{0.82,1.00},{0.18,1.00}}},
  left ={k1=-0.14,k2=0.030,flipU=false,flipV=false,rotate=-90,
    crop={u0=0.02,v0=0.23,u1=0.98,v1=0.96},
    quad={{0.00,0.10},{0.40,0.36},{0.40,0.64},{0.00,0.90}}},
  right={k1=-0.14,k2=0.030,flipU=true, flipV=false,rotate=90,
    crop={u0=0.02,v0=0.23,u1=0.98,v1=0.96},
    quad={{0.60,0.36},{1.00,0.10},{1.00,0.90},{0.60,0.64}}}
}
...
local function emitStatus(state,msg)
  if guihooks and guihooks.trigger then
    guihooks.trigger('SurroundViewStatus',{state=state,message=msg or state or 'UNKNOWN',mode='calibrated-four-camera-cellwarp',version='0.7.3'})
  end
end
...
local function updateCams()
  ...
  local ok,err=pcall(function()
    -- Slightly higher and more downward-looking than v0.7.2 to reduce the extreme near-ground zoom.
    setCam(cameras.front,p+d*2.25+u*1.05, d-u*0.55,u)
    setCam(cameras.rear, p-d*2.25+u*1.05,-d-u*0.55,u)
    setCam(cameras.left, p-r*1.20+u*1.05,-r-u*0.58,u)
    setCam(cameras.right,p+r*1.20+u*1.05, r-u*0.58,u)
  end)
...
-- AddImage is stable on retail BeamNG. Preserve UV direction (including reversed UVs)
-- instead of sorting min/max, otherwise mirrored camera cells become scrambled.
local function warpCells(dl,cam,c,o,w,h)
  ...
  for gy=0,GRID-1 do
    ...
    for gx=0,GRID-1 do
      ...
      local minx=math.min(x1,x2,x3,x4)-1.0; local maxx=math.max(x1,x2,x3,x4)+1.0
      local miny=math.min(y1,y2,y3,y4)-1.0; local maxy=math.max(y1,y2,y3,y4)+1.0

      -- Use the UVs at the cell corners without min/max reordering. This keeps flips/rotations coherent.
...
local function surround()
  local a=im.GetContentRegionAvail(); local s=math.max(280,math.min(a.x,a.y,720)); local w,h=s*0.78,s
  ...
  local cw,ch=w*0.22,h*0.40; local cx,cy=o.x+w*0.5,o.y+h*0.5
  ...
  local glass0=im.ImVec2(cx-cw*0.30,cy-ch*0.26)
  local glass1=im.ImVec2(cx+cw*0.30,cy+ch*0.20)
  dl:AddRectFilled(glass0,glass1,im.GetColorU322(im.ImVec4(0.06,0.11,0.15,1)),cw*0.18,0)
...
local function calibrationPanel()
  im.Text('v0.7.3 calibration: UV orientation + side rotation + wider bird-eye'); im.Separator()
...
local function drawWindow()
  ...
  if open then
    im.Text('4x GPU cameras / improved calibrated bird\'s-eye v0.7.3'); im.Separator()
    if im.BeginTabBar('svTabs') then
      if im.BeginTabItem('360 VIEW') then
        im.Text('Live bird\'s-eye composite - UV/rotation corrected')
...
function M.startSurroundView()
  ...
  emitStatus('live','CALIBRATED 4 CAMERA SURROUND LIVE v0.7.3'); return true
end
